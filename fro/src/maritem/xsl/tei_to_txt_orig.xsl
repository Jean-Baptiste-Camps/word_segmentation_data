...
<xsl:stylesheet 
    ...
    <xsl:template match="/">
        ...
        inside the front and back. -->
                    <xsl:apply-templates select="descendant::tei:body/descendant::tei:ab"/>
                </xsl:result-document>
            </xsl:when>
            <xsl:when test="$decoupage = 'par-feuillet'">
                <xsl:for-each select="descendant::tei:body/descendant::tei:pb[not(matches(@n,'v'))]">
                    ...
                    <xsl:result-document href="{$filename}_{$n}.txt">
                        ...
                        <xsl:apply-templates
                            select="following::tei:ab[generate-id(preceding::tei:pb[1]) = $pbid or generate-id(preceding::tei:pb[2]) = $pbid]"
                        />
                    </xsl:result-document>
                </xsl:for-each>
            </xsl:when>
            <xsl:when test="$decoupage = 'par-recto'">
                <xsl:for-each select="descendant::tei:pb">
                    ...
                    <xsl:result-document href="{$filename}_{$n}.txt">
                        <xsl:variable name="pbid" select="generate-id(.)"/>
                        <xsl:apply-templates
                            select="following::tei:ab[generate-id(preceding::tei:pb[1]) = $pbid]"
                        />
                    </xsl:result-document>
                </xsl:for-each>
            </xsl:when>
            <xsl:when test="$decoupage = 'par-colonne'">
                <xsl:for-each select="descendant::tei:cb">
                    ...
                    <xsl:result-document href="{$filename}_{$n}.txt">
                        ...
                        <xsl:apply-templates
                            select="following::tei:ab[generate-id(preceding::tei:cb[1]) = $cbid]"
                        />
                    </xsl:result-document>
                </xsl:for-each>
            </xsl:when>
        ...
        <!--        <xsl:for-each-group select="descendant::tei:ab" group-starting-with="tei:pb">
            <xsl:result-document href="{$filename}_{current-group()/tei:pb/@n}.txt">
                <xsl:apply-templates select="current-group()"/>
            </xsl:result-document>
        </xsl:for-each-group>      -->

        <!--<xsl:apply-templates select="descendant::tei:ab"/>-->
    </xsl:template>
   ...
   <!-- <xsl:template match="tei:orig[. = '&#769;']"/>
    ...
    <xsl:template match="tei:w">
        ...
        <xsl:if
            test="generate-id(.) != generate-id(ancestor::tei:ab/descendant::tei:w[position() = last()]) and not(@rend)">
            <xsl:text> </xsl:text>
        </xsl:if>
    </xsl:template>
